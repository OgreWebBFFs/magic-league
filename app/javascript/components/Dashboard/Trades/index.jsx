...
const EmptyState = () => (
  <div className="empty-card-view">
    <p className="empty-card-view__message">
      No trades here yet! Someone better start wheelin&#39; and dealin&#39;
    </p>
  </div>
);

const DateStatusCell = ({ children }) => <Cell className="trades-table-cell__date-status">{children}</Cell>;
const CardListCell = ({ children }) => <Cell className="trades-table-cell__card-list" isPriority>{children}</Cell>;
const NameCell = ({ children }) => <Cell className="trades-table-cell__name" isPriority>{children}</Cell>;
