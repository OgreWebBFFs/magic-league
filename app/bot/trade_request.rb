class TradeRequest
  ...
  def ask_about_card to_user, from_user, card, wishlist_cards
    msg = "#{from_user.name} is interested in trading for your #{card.discord_link} (#{card.rarity[0].upcase})"
    if (wishlist_cards.length > 0)
      msg += "\n\nThey own the following cards on your wishlist\n"
      msg += wishlist_cards.map {|card| "- #{card.discord_link} (#{card.rarity[0].upcase})"}.join("\n")
    end
